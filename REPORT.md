...
**双向认证（mutual pq-auth）**：上述 pq-auth 仅认证网关。进一步为客户端也配长期 ML-DSA 身份，
客户端在 ClientFinished 中附带对 `("client-auth-v1" ∥ TH1)` 的签名（`client_authenticator` 字段），
网关用预置的客户端公钥验证，即实现双向认证。实验对三类客户端的结果如下：

| 客户端类型 | 握手结果 | 网关是否接受 |
|-----------|:--------:|:----------:|
| 合法客户端（正确长期密钥） | success | 接受 |
| 冒充客户端（错误长期密钥） | fail | 拒绝 |
| 未认证客户端（不带签名） | fail | 拒绝 |

> 仅持正确长期密钥的客户端被接受，冒充与未认证客户端均被网关拒绝，双向认证成立。
> 代价为客户端一次签名、网关一次验签（各约数毫秒），ClientFinished 增加约 3.3 KB。
> 命令：`python main.py auth`（依次输出冒充网关对比与双向认证结果）。
...
1. **长期身份认证（已由可选 pq-auth 闭环，含双向）**：默认模式下 `gateway_authenticator` 只是密钥确认，
   无法抵抗完整冒充网关的主动中间人（见 §4.4）。§6.6 的 pq-auth 模式用 ML-DSA 长期签名挡住了该攻击，
   并进一步实现了双向认证（网关也验证客户端长期签名，冒充/未认证客户端均被拒）。
   仍依赖"双方预置对端公钥（pinning）"这一信任锚，未实现完整的 PKI / 证书链 / 密钥吊销。
